...
function gitChanged(patterns){
  try {
    ...
    const hasCommit = execSync('git rev-parse --verify HEAD', { cwd: root, stdio: 'pipe' }).toString().trim().length > 0;
    const baseList = hasCommit ? execSync('git diff --name-only HEAD', { cwd: root, stdio: 'pipe' }).toString().split('\n') : [];
    const staged = execSync('git diff --name-only --cached', { cwd: root, stdio: 'pipe' }).toString().split('\n');
...
function matchGlob(pattern, file){
  const regex = new RegExp('^' + pattern.split(/[-\\^$+?.()|{}]/g).map(seg => seg.replace(/\*\*/g, '(?:(?:.+/)?|)').replace(/\*/g, '[^/]*')).join('') + '$');
...
if (forceFull) {
  ...
  if (gitChanged(['public/**','scripts/version-assets.mjs','firebase.json'])) firebaseTargets.add('hosting');
  if (gitChanged(['functions/**'])) firebaseTargets.add('functions');
  if (gitChanged(['firestore.rules','firestore.indexes.json'])) firebaseTargets.add('firestore');
...
if (forceWorker) deployWorker = true; else if (gitChanged(['cloudflare/worker.js','cloudflare/wrangler.toml'])) deployWorker = true;
